...
const TaskCard = ({ task, onStatusUpdate }) => {
    ...
    return (
        <div className="staff-card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', border: '1px solid rgba(255,255,255,0.05)' }}>
            <div style={{ display: 'flex', gap: '24px', alignItems: 'center' }}>
                ...
                <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', margin: '0 0 4px' }}>
                        <h3 style={{ margin: 0, fontSize: '1.1rem' }}>{task.serviceType}</h3>
                        <span className={`status-badge status-${task.status?.toLowerCase()}`} style={{ fontSize: '0.7rem', padding: '2px 8px' }}>
                            {task.status}
                        </span>
                    </div>
                    <div style={{ display: 'flex', gap: '15px', fontSize: '0.85rem', color: 'var(--staff-text-soft)', flexWrap: 'wrap', marginBottom: '4px' }}>
                        <span>👤 {task.customerName}</span>
                        <span style={{ display: 'flex', alignItems: 'center', gap: '4px' }}><Phone size={12}/> {task.customerPhone}</span>
                        <span>🚗 {task.vehiclePlate}</span>
                        <span>📅 {task.bookingDate} {task.bookingTime}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '6px', fontSize: '0.85rem', color: '#cbd5e1', marginBottom: '6px' }}>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
                {task.status === 'SUCCESS' && (
                    <>
                        <button className="staff-primary-btn" style={{ background: 'rgba(239, 68, 68, 0.2)', color: '#ef4444', border: '1px solid #ef4444' }} onClick={(e) => {
                            e.stopPropagation();
                        ...
                        <button className="staff-primary-btn" onClick={(e) => { e.stopPropagation(); onStatusUpdate(task.id, 'NEXT'); }}>
                            Bắt đầu làm
                        </button>
                    </>
                )}
