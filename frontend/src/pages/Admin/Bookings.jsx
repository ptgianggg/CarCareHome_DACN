...
function BookingManagement() {
  ...
  const handleAssignStaff = async () => {
    ...
    try {
        await assignStaff(detailBooking.id, selectedStaffId);
        await fetchData();
        ...
        <article className="panel" style={{ padding: '0', background: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '24px', overflow: 'hidden' }}>
            ...
            <div style={{ display: 'flex', gap: '20px', flex: 1, minWidth: '400px' }}>
              ...
              <div style={{ position: 'relative' }} ref={statusRef}>
                <div 
                  ...
                  style={{ 
                    height: '60px', minWidth: '240px', background: 'rgba(0,0,0,0.4)', 
                ...
                {isStatusOpen && (
                  <div style={{ 
                    ...
                    borderRadius: '18px', padding: '8px', zIndex: 100
                  }}>
                    {STATUS_OPTIONS.map((opt) => (
                      <div 
                        ...
                        style={{ 
                          ...
                          alignItems: 'center', gap: '12px', color: filterStatus === opt.value ? '#3b82f6' : '#94a3b8',
          ...
          <div className="booking-index-table" style={{ padding: '10px 0' }}>
            ...
            {!loading && paginatedBookings.map((b) => (
              <BookingRow key={b.id} b={b} onDoubleClick={setDetailBooking} />
            ))}
          </div>
          ...
          {!loading && totalPages > 1 && (
            <div style={{ padding: '30px', borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button onClick={() => setCurrentPage(c => Math.max(1, c-1))} className="page-btn">{"<"}</button>
                {[...Array(totalPages)].map((_, i) => (
                  <button key={i} onClick={() => setCurrentPage(i+1)} className={`page-btn ${currentPage === i+1 ? 'active' : ''}`}>{i+1}</button>
                ))}
                <button onClick={() => setCurrentPage(c => Math.min(totalPages, c+1))} className="page-btn">{">"}</button>
              </div>
            </div>
          )}
        </article>
